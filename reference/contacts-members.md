...
## Gotchas / LINE login button on non-checkout pages (DEV-1801)
The **"เข้าสู่ระบบด้วย LINE"** (Login with LINE) button inside the member login box that appears on **general pages** (e.g. Contact Us — rendered via `Views/Component/MemberCtrl/`) uses `ng-click="lineLogin()"`. Before the DEV-1801 fix, `$scope.lineLogin` was not declared in `Component/Member/Controller.js` (only in the Global/Shopcart controller), so the button silently did nothing. The fix imports `lineLogin` from `SocialLogin` into `Component/Member/Controller.js`. If LINE login still appears broken on a general page (not checkout), confirm the tenant has **Enable login with Social Media > LINE** configured in `?manage=true#!/WebConfig` General tab.

## Gotchas / register popup form CSS classes (feature/member-register-popup-match-form)
The company-info fields in the member register popup (`Views/Component/MemberCtrl/MemberRegister.cshtml`) use CSS classes `itp-member-company-name`, `itp-member-branch-number`, and `itp-member-tax-id` on their wrapper `<div>`s. These classes match the structure in the standalone `Register.cshtml`. Tenant CSS that targets these classes will apply to the popup form as well.
